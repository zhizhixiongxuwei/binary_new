...
function newIdempotencyKey(): string {
  if (typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function') {
    return `java-analysis-${crypto.randomUUID()}`
  }
  return `java-analysis-${Date.now()}-${Math.random().toString(16).slice(2)}`
}
...
export function useJavaAnalysis(options: UseJavaAnalysisOptions) {
  const projects = shallowRef<readonly DecompileProject[]>([])
  const runs = shallowRef<readonly JavaAnalysisRun[]>([])
  const findings = shallowRef<readonly JavaAnalysisFinding[]>([])
  const selectedProjectId = shallowRef('')
  const selectedRunId = shallowRef('')
  const selectedFinding = shallowRef<JavaAnalysisFinding>()
  const nextFindingCursor = shallowRef('')
  const filters = shallowRef<JavaAnalysisFilters>({
    cwe: '',
    file: '',
    callable: '',
  })
  ...
  function replaceRun(next: JavaAnalysisRun): void {
  ...
  async function pollActiveRun(): Promise<void> {
    ...
    try {
      const refreshed = await api.getJavaAnalysisRun(taskId, current.id)
      if (
      ...
      if (
        ...
        if (preferredProject) {
          if (
            ...
            throw new Error('所选源码项目不支持 Java 检测')
          }
          loadedProjects = [
            preferredProject,
  ...
  async function loadFindings(append = false): Promise<void> {
    ...
    try {
      ...
      const page = await api.listJavaAnalysisFindings(taskId, run.id, {
  ...
  function applyFilters(next: JavaAnalysisFilters): void {
    filters.value = {
  ...
  async function createRun(): Promise<void> {
    ...
    try {
      const run = await api.createJavaAnalysisRun(
        taskId,
        project.id,
        requestContext.key,
      )
  ...
  async function cancelRun(): Promise<void> {
    ...
    operationError.value = ''
    try {
      const refreshed = await api.cancelJavaAnalysisRun(taskId, run.id)
      if (
        currentGeneration !== generation ||
        taskId !== toValue(options.taskId) ||
  ...
  async function deleteRun(): Promise<boolean> {
    ...
    operationError.value = ''
    try {
      await api.deleteJavaAnalysisRun(taskId, run.id)
      if (
        currentGeneration !== generation ||
        taskId !== toValue(options.taskId) ||
        projectId !== selectedProjectId.value ||
  ...
  function selectFinding(finding?: JavaAnalysisFinding): void {
