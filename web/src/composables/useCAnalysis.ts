...
function newIdempotencyKey(): string {
  if (typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function') {
    return `c-analysis-${crypto.randomUUID()}`
  }
  return `c-analysis-${Date.now()}-${Math.random().toString(16).slice(2)}`
}
...
export function useCAnalysis(options: UseCAnalysisOptions) {
  const projects = shallowRef<readonly DecompileProject[]>([])
  const runs = shallowRef<readonly CAnalysisRun[]>([])
  const findings = shallowRef<readonly CAnalysisFinding[]>([])
  const selectedProjectId = shallowRef('')
  const selectedRunId = shallowRef('')
  const selectedFinding = shallowRef<CAnalysisFinding>()
  const nextFindingCursor = shallowRef('')
  const filters = shallowRef<CAnalysisFilters>({ cwe: '', function: '' })
  ...
  const state = computed<TaskResultState>(() => {
    if (loading.value) {
      return {
        status: 'loading',
        title: '正在读取 C 源码检测',
        description: '正在加载可检测的 Ghidra 源码项目和历史运行。',
      }
    }
    if (error.value) {
      return {
    ...
    if (projects.value.length === 0) {
      return {
        status: 'empty',
        title: '暂无可检测的 Ghidra 源码项目',
        description: '请先完成 Ghidra 反编译并保存 project-v1 类 C 源码。',
      }
    }
    return { status: 'ready' }
  })
  ...
  function replaceRun(next: CAnalysisRun): void {
  ...
  async function pollActiveRun(): Promise<void> {
    ...
    try {
      const refreshed = await api.getCAnalysisRun(taskId, current.id)
      if (
  ...
  async function load(): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        if (preferredProject) {
          if (
            ...
            throw new Error('所选源码项目不支持 C 检测')
          }
          loadedProjects = [
            preferredProject,
      ...
      error.value = messageFor(caught, 'C 源码检测读取失败')
  ...
  async function loadFindings(append = false): Promise<void> {
    ...
    try {
      ...
      const page = await api.listCAnalysisFindings(taskId, run.id, {
        ...
        ...(activeFilters.function
          ? { function: activeFilters.function }
          : {}),
      })
      if (
  ...
  function applyFilters(next: CAnalysisFilters): void {
    filters.value = {
      cwe: next.cwe.trim().toUpperCase(),
      ...(next.severity ? { severity: next.severity } : {}),
      function: next.function.trim(),
    }
    selectedFinding.value = undefined
    void loadFindings()
  }
  ...
  async function createRun(): Promise<void> {
    ...
    try {
      const run = await api.createCAnalysisRun(
        taskId,
        project.id,
        requestContext.key,
      )
  ...
  async function cancelRun(): Promise<void> {
    ...
    operationError.value = ''
    try {
      const refreshed = await api.cancelCAnalysisRun(taskId, run.id)
      if (
        currentGeneration !== generation ||
        taskId !== toValue(options.taskId) ||
  ...
  async function deleteRun(): Promise<boolean> {
    ...
    operationError.value = ''
    try {
      await api.deleteCAnalysisRun(taskId, run.id)
      if (
        currentGeneration !== generation ||
        taskId !== toValue(options.taskId) ||
        projectId !== selectedProjectId.value ||
  ...
  function selectFinding(finding?: CAnalysisFinding): void {
