...
const emit = defineEmits<{
  select: [finding: JavaAnalysisFinding]
  loadMore: []
}>()

function locationLabel(finding: JavaAnalysisFinding): string {
...
<template>
  ...
  <template v-else>
    <div class="finding-table-scroll" tabindex="0" aria-label="Java 源码检测结果表格滚动区域">
      <table class="finding-table">
        ...
        <tbody>
          ...
          >
            ...
            <td>
              <strong class="file-path" :title="finding.file.logical_path">
                {{ finding.file.logical_path }}
              </strong>
              <small :title="finding.file.binary_name">{{ finding.file.binary_name }}</small>
            </td>
            <td>
              <strong class="type-name" :title="finding.callable.type_name">
                {{ finding.callable.type_name }}
              </strong>
            </td>
            <td>
              <strong class="callable-name" :title="finding.callable.name">
                {{ finding.callable.name }}
              </strong>
              <small :title="finding.callable.signature">
                {{ finding.callable.kind }}<template v-if="finding.callable.signature"> · {{ finding.callable.signature }}</template>
              </small>
            </td>
            ...
            <td class="finding-actions">
              <button
                type="button"
                title="查看代码片段"
                :aria-label="`查看 ${finding.callable.name} 的代码片段`"
...
.finding-table th:nth-child(4) { width: 220px; }
.finding-table th:nth-child(5) { width: 160px; }
.finding-table th:nth-child(6) { width: 200px; }
.finding-table th:nth-child(7) { width: 82px; }
.finding-table th:nth-child(9) { width: 58px; }
...
.file-path,
.type-name,
.callable-name { color: var(--ink-800); font-size: 10px; }
