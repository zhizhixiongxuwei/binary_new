...
import type { PythonAnalysisRun, PythonAnalysisSeverity } from '@/api/types'
import { formatBytes, formatDateTime } from '@/utils/formatters'

const props = defineProps<{
  run: PythonAnalysisRun
}>()

const statusLabels: Readonly<Record<PythonAnalysisRun['status'], string>> = {
  queued: '排队中',
  running: '检测中',
  succeeded: '已完成',
  partial: '部分完成',
...
<template>
  ...
  <div v-if="run.error_message" class="analysis-error" role="alert">
    ...
    <span><code>{{ run.error_code || 'python_analysis_failed' }}</code>{{ run.error_message }}</span>
  </div>
</template>
