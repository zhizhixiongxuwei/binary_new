...
import type { CAnalysisRun, CAnalysisSeverity } from '@/api/types'
...
const statusLabels: Readonly<Record<CAnalysisRun['status'], string>> = {
  queued: '排队中',
  running: '检测中',
  succeeded: '已完成',
  partial: '部分完成',
...
const coveragePercent = computed(() =>
  props.run.coverage.total_functions > 0
    ? Math.round(
        (props.run.coverage.parsed_functions /
          props.run.coverage.total_functions) *
          100,
      )
    : 0,
)
...
<template>
  <section class="analysis-summary" aria-label="C 源码检测汇总">
    ...
    <div class="analysis-summary__coverage">
      ...
      <span>
        <small>函数覆盖</small>
        <strong>{{ run.coverage.parsed_functions }}/{{ run.coverage.total_functions }} · {{ coveragePercent }}%</strong>
      </span>
    </div>
  </section>

  <section class="analysis-meta" aria-label="C 源码检测版本信息">
    ...
    <span><small>源码</small><code>{{ formatBytes(run.source_size_bytes) }} · {{ run.source_sha256.slice(0, 12) }}</code></span>
  ...
  >
    ...
    <span>
      检测结果不完整：{{ run.coverage.failed_functions }} 个函数未成功解析<span v-if="run.findings_truncated">，发现数量达到上限</span><span v-if="run.diagnostics_truncated">，诊断数量达到上限</span>。
    </span>
  </div>

  <div v-if="run.error_message" class="analysis-error" role="alert">
    <AlertTriangle :size="16" aria-hidden="true" />
    <span><code>{{ run.error_code || 'c_analysis_failed' }}</code>{{ run.error_message }}</span>
  </div>
</template>
