...
import { Code2, ExternalLink, MapPin, ShieldAlert } from 'lucide-vue-next'
...
const snippetLines = computed<readonly SnippetLine[]>(() => {
  ...
  const hasReliableSourceLine = lines.length === 1
  return lines.map((text, index) => ({
    key: index,
    label: hasReliableSourceLine
      ? String(finding.location.start_line)
      : `S${index + 1}`,
    labelTitle: hasReliableSourceLine
      ? `源码第 ${finding.location.start_line} 行`
      : `片段内第 ${index + 1} 行`,
    text,
    hit: hasReliableSourceLine,
  }))
})
</script>
...
<template>
  ...
  >
    <article v-if="finding" class="finding-detail">
      ...
      <dl>
        <div>
          <dt><Code2 :size="13" aria-hidden="true" />函数</dt>
          <dd>
            <strong>{{ finding.function.name }}</strong>
            <code>{{ finding.function.address || '地址不可用' }}</code>
          </dd>
        </div>
        <div>
      ...
      <el-button
        type="primary"
        plain
        @click="emit('openSource', finding.function.result_id)"
      >
