import type {
  ...
  PythonAnalysisRunStatus,
  PythonAnalysisSeverity,
} from '@/api/types'

const runStatuses = new Set<PythonAnalysisRunStatus>([
  'queued',
  'running',
  'succeeded',
  'partial',
...
const severities = new Set<PythonAnalysisSeverity>([
  'LOW',
  'MEDIUM',
  'HIGH',
  'CRITICAL',
...
export class PythonAnalysisContractError extends Error {
  constructor(field: string) {
    super(`Java 源码检测响应不符合接口契约：${field}`)
    this.name = 'PythonAnalysisContractError'
  }
}

function record(value: unknown, field: string): Record<string, unknown> {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new PythonAnalysisContractError(field)
  }
...
function uuid(value: unknown, field: string): string {
  ...
  if (!uuidPattern.test(parsed)) throw new PythonAnalysisContractError(field)
  return parsed
}
...
function positiveInteger(value: unknown, field: string, maximum: number): number {
  ...
  if (parsed === 0) throw new PythonAnalysisContractError(field)
  return parsed
}

function booleanValue(value: unknown, field: string): boolean {
  if (typeof value !== 'boolean') throw new PythonAnalysisContractError(field)
  return value
}

function timestamp(value: unknown, field: string): string {
  const parsed = textValue(value, field, 64)
  if (!Number.isFinite(Date.parse(parsed))) throw new PythonAnalysisContractError(field)
  return parsed
}
...
function enumValue<T extends string>(
  ...
  const parsed = textValue(value, field, 64)
  if (!allowed.has(parsed as T)) throw new PythonAnalysisContractError(field)
  return parsed as T
}
...
function runValue(value: unknown, field: string): PythonAnalysisRun {
  ...
  if (totalFiles !== sourceFileCount) {
    throw new PythonAnalysisContractError(`${field}.coverage.total_files`)
  }
  ...
  if (
    ...
    throw new PythonAnalysisContractError(`${field}.coverage`)
  }
  ...
  if (
    ...
    throw new PythonAnalysisContractError(`${field}.severity_counts`)
  }
  ...
  if (sourceProjectID !== projectID) {
    throw new PythonAnalysisContractError(`${field}.source_project.id`)
  }
...
function findingValue(value: unknown, field: string): PythonAnalysisFinding {
  ...
  if (endLine < startLine || (endLine === startLine && endColumn < startColumn)) {
    throw new PythonAnalysisContractError(`${field}.location`)
  }
  const id = textValue(finding.id, `${field}.id`, 20)
  if (!positiveDecimalPattern.test(id)) {
    throw new PythonAnalysisContractError(`${field}.id`)
  }

  const parsed: PythonAnalysisFinding = {
    id,
  ...
  if (hasSnippet !== hasSnippetStartLine) {
    throw new PythonAnalysisContractError(`${field}.snippet`)
  }
  if (hasSnippet) {
...
function pageValue<T>(
  ...
  if (!Array.isArray(page.items) || page.items.length > 200) {
    throw new PythonAnalysisContractError(`${field}.items`)
  }
...
export function parsePythonAnalysisRun(value: unknown): PythonAnalysisRun {
  return runValue(value, 'data')
}

export function parsePythonAnalysisRunPage(value: unknown): PythonAnalysisRunPage {
  return pageValue(value, 'data', runValue)
}

export function parsePythonAnalysisFindingPage(
  value: unknown,
): PythonAnalysisFindingPage {
