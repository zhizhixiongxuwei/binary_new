...
所有“新建任务”入口统一先选择 `01 二进制格式`、`02 压缩包格式` 或 `03 容器镜像格式`。浏览器只对能够高置信识别的文件头做提前拦截，服务端仍按文件内容执行最终格式校验；扩展名、MIME 和用户声明都不能覆盖检测结果。类别不匹配或不支持的文件不会创建任务，并可从上传队列直接删除。

`01` 支持 PE、ELF、Mach-O、CLASS、JAR、WAR、EAR、DEX、APK 和 PYC；`03` 支持 Docker Save TAR 与 OCI Image Layout TAR。两类都允许同类别多文件上传，校验通过后每个文件分别创建一个任务。EXT2/3/4、SquashFS、ISO9660、UDF 和 MBR/GPT 不属于新建任务支持范围。

`02` 支持 ZIP、7Z、RAR、TAR、GZIP、BZIP2、XZ、ZSTD、CAB、CPIO、AR、DEB 和 RPM。每个外层压缩包作为独立导入批次异步解析，先展示内部候选文件，再由用户每次最多选择 20 项创建任务。TAR.GZ 和 DEB/RPM 可以展开格式固有包装，但成员中的普通嵌套压缩包不会递归；成员若识别为 JAR/APK 等二进制或 Docker/OCI 镜像，仍可创建对应任务。加密包、分卷包和无有效成员的压缩包不会创建任务，并提供删除入口。
