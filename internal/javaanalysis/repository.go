...
const jobPayloadSchemaVersion = "binaryscan-java-analysis-job/v1"
...
func NewMySQLRepository(
	...
	if db == nil {
		return nil, errors.New("Java analysis database is required")
	}
	...
	if config.AnalyzerVersion != AnalyzerVersion ||
		...
		return nil, errors.New("Java analysis repository configuration is invalid")
	}
...
func (r *MySQLRepository) Create(
	...
	if err != nil {
		return Run{}, false, fmt.Errorf("begin Java analysis creation: %w", err)
	}
	defer tx.Rollback()
...
FOR UPDATE`, record.TaskID).Scan(&taskStatus, &taskDeleted)
	...
	if err != nil {
		return Run{}, false, fmt.Errorf("lock Java analysis task: %w", err)
	}
...
FOR UPDATE`, record.TaskID, record.RequestKey).Scan(
	...
	if err == nil {
		...
		if err != nil {
			return Run{}, false, fmt.Errorf("read replayed Java analysis: %w", err)
		}
		if err := tx.Commit(); err != nil {
			return Run{}, false, fmt.Errorf("commit Java analysis replay: %w", err)
		}
		return value, false, nil
	}
	if !errors.Is(err, sql.ErrNoRows) {
		return Run{}, false, fmt.Errorf("find Java analysis replay: %w", err)
	}
...
SELECT EXISTS (
    ...
    WHERE worker_kind = 'java_analysis'
      AND analyzer_name = ?
      ...
      AND last_checked_at >= DATE_SUB(
		...
		return Run{}, false, fmt.Errorf("check Java analysis readiness: %w", err)
	}
	if !ready {
...
FOR UPDATE`, record.SourceProjectID).Scan(&activeID)
	...
	if !errors.Is(err, sql.ErrNoRows) {
		return Run{}, false, fmt.Errorf("find active Java analysis: %w", err)
	}
	...
	if err != nil {
		return Run{}, false, fmt.Errorf("encode Java analysis job payload: %w", err)
	}
...
INSERT INTO jobs (
    ...
    ?, ?, ?, 'java_analysis', 'queued', 10, ?, UTC_TIMESTAMP(6),
    0, 3, 0, ?
)`, record.JobID, record.TaskID, attemptID, payload, record.RequestKey); err != nil {
		return Run{}, false, fmt.Errorf("insert Java analysis job: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `
...
INSERT INTO java_analysis_runs (
    ...
    source_file_count
		...
		return Run{}, false, fmt.Errorf("insert Java analysis run: %w", err)
	}
	value, err := scanRun(tx.QueryRowContext(
		ctx, runSelect+` WHERE analysis.task_id = ? AND analysis.id = ?`,
	...
	if err != nil {
		return Run{}, false, fmt.Errorf("read created Java analysis: %w", err)
	}
	if err := r.invalidateReports(ctx, tx, record.TaskID); err != nil {
		return Run{}, false, fmt.Errorf(
			"invalidate reports after Java analysis creation: %w", err,
		)
	}
	if err := tx.Commit(); err != nil {
		return Run{}, false, fmt.Errorf("commit Java analysis creation: %w", err)
	}
...
func (r *MySQLRepository) List(
	...
	if err != nil {
		return RunPage{}, fmt.Errorf("begin Java analysis list: %w", err)
	}
	defer tx.Rollback()
  ...
  AND (analysis.created_at < ? OR
	...
	if err != nil {
		return RunPage{}, fmt.Errorf("list Java analysis runs: %w", err)
	}
	...
	for rows.Next() {
		...
		if err != nil {
			rows.Close()
			return RunPage{}, fmt.Errorf("scan Java analysis run: %w", err)
		}
		items = append(items, value)
	}
	if err := rows.Err(); err != nil {
		rows.Close()
		return RunPage{}, fmt.Errorf("iterate Java analysis runs: %w", err)
	}
	if err := rows.Close(); err != nil {
		return RunPage{}, fmt.Errorf("close Java analysis runs: %w", err)
	}
	...
	if err := tx.Commit(); err != nil {
		return RunPage{}, fmt.Errorf("commit Java analysis list: %w", err)
	}
	return page, nil
}
...
func (r *MySQLRepository) Get(
	...
	if err != nil {
		return Run{}, fmt.Errorf("begin Java analysis read: %w", err)
	}
	defer tx.Rollback()
	if err := requireTask(ctx, tx, query.TaskID); err != nil {
		return Run{}, err
	...
	if err != nil {
		return Run{}, fmt.Errorf("read Java analysis run: %w", err)
	}
	if err := tx.Commit(); err != nil {
		return Run{}, fmt.Errorf("commit Java analysis read: %w", err)
	}
	return value, nil
}
...
WHERE analysis.task_id = ? AND analysis.id = ?
  AND analysis.deletion_started_at IS NULL`,
	...
	if err != nil {
		return FindingPage{}, fmt.Errorf("find Java analysis run for findings: %w", err)
	}
	statement := `
...
WHERE task_id = ? AND run_id = ? AND id > ?`
	...
	rows, err := tx.QueryContext(ctx, statement, arguments...)
	if err != nil {
		return FindingPage{}, fmt.Errorf("list Java analysis findings: %w", err)
	}
	...
	for rows.Next() {
		...
		if err != nil {
			rows.Close()
			return FindingPage{}, fmt.Errorf("scan Java analysis finding: %w", err)
		}
		items = append(items, value)
		ids = append(ids, id)
	}
	if err := rows.Err(); err != nil {
		rows.Close()
		return FindingPage{}, fmt.Errorf("iterate Java analysis findings: %w", err)
	}
	if err := rows.Close(); err != nil {
		return FindingPage{}, fmt.Errorf("close Java analysis findings: %w", err)
	}
...
func (r *MySQLRepository) Cancel(
	...
	tx, err := r.db.BeginTx(ctx, &sql.TxOptions{Isolation: sql.LevelReadCommitted})
	if err != nil {
		return Run{}, fmt.Errorf("begin Java analysis cancellation: %w", err)
	}
	defer tx.Rollback()
...
FOR UPDATE`, input.TaskID, input.RunID).Scan(&runStatus, &jobID, &jobStatus)
	...
	if err != nil {
		return Run{}, fmt.Errorf("lock Java analysis cancellation: %w", err)
	}
	mutated := false
	switch runStatus {
...
WHERE id = ? AND task_id = ? AND status = 'queued'`, jobID, input.TaskID); err != nil {
				return Run{}, fmt.Errorf("cancel queued Java analysis job: %w", err)
			}
	...
	if err != nil {
		return Run{}, fmt.Errorf("read cancelled Java analysis: %w", err)
	}
	if mutated {
		if err := r.invalidateReports(ctx, tx, input.TaskID); err != nil {
			return Run{}, fmt.Errorf(
				"invalidate reports after Java analysis cancellation: %w", err,
			)
		}
	}
	if err := tx.Commit(); err != nil {
		return Run{}, fmt.Errorf("commit Java analysis cancellation: %w", err)
	}
	return value, nil
}
...
WHERE id = ? AND task_id = ?
  AND status IN ('leased', 'running', 'cancel_requested')`,
		...
		return fmt.Errorf("request Java analysis job cancellation: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `
...
WHERE task_id = ? AND id = ? AND status IN ('queued', 'running', 'cancel_requested')`,
		...
		return fmt.Errorf("request Java analysis run cancellation: %w", err)
	}
	return nil
}
...
WHERE task_id = ? AND id = ? AND status = 'queued'`,
		...
		return fmt.Errorf("cancel Java analysis run: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `
...
func lockEligibleProject(
	...
	var sourceFileCount, symbolCount, projectSourceSize uint64
	var rootKey, manifestKey, manifestSHA sql.NullString
	var manifestSize sql.Null[uint64]
	err := tx.QueryRowContext(ctx, `
SELECT project.status, project.source_kind, project.language,
       project.engine_name, project.engine_version,
       project.root_storage_key, project.manifest_storage_key,
       project.manifest_sha256, project.manifest_size_bytes,
       project.source_file_count, project.symbol_count, project.source_size_bytes,
...
FOR UPDATE`, taskID, projectID).Scan(
		&project.Status, &project.SourceKind, &project.Language,
		&project.EngineName, &project.EngineVersion,
		&rootKey, &manifestKey, &manifestSHA, &manifestSize,
		&sourceFileCount, &symbolCount, &projectSourceSize,
		&attemptID, &fileNodeID,
		&project.TargetPath,
	)
	if errors.Is(err, sql.ErrNoRows) {
	...
	if err != nil {
		return ProjectSnapshot{}, 0, 0, fmt.Errorf("lock Java source project: %w", err)
	}
	...
	project.ProjectSymbolCount = symbolCount
	...
	if !attemptID.Valid || attemptID.V == 0 || fileNodeID == 0 ||
		...
		!manifestKey.Valid || manifestKey.String != path.Join(expectedRoot, "manifest.json") ||
		!manifestSHA.Valid || !sha256Pattern.MatchString(manifestSHA.String) ||
		!manifestSize.Valid || manifestSize.V == 0 ||
		manifestSize.V > uint64(MaxManifestBytes) || sourceFileCount == 0 ||
		symbolCount == 0 || projectSourceSize == 0 {
		return ProjectSnapshot{}, 0, 0, ErrSourceUnavailable
	}
	rows, err := tx.QueryContext(ctx, `
SELECT result.id,
       SUBSTRING(result.storage_key, CHAR_LENGTH(?) + 2),
       JSON_UNQUOTE(JSON_EXTRACT(result.diagnostics_json, '$.binary_name')),
       result.content_sha256, result.size_bytes
FROM decompile_results result
WHERE result.task_id = ? AND result.analyzer_run_id = ?
	  AND result.status = 'complete' AND result.language = 'java'
	  AND result.deleted_at IS NULL AND result.storage_key IS NOT NULL
	  AND result.storage_key LIKE CONCAT(?, '/src/main/java/%')
	  AND result.size_bytes > 0
ORDER BY result.storage_key ASC, result.id ASC`,
		expectedRoot, taskID, projectID, expectedRoot,
	)
	if err != nil {
		return ProjectSnapshot{}, 0, 0, fmt.Errorf("list Java source files: %w", err)
	}
	var previousPath string
	for rows.Next() {
		var file SourceFile
		var logicalPath, binaryName, digest sql.NullString
		var size sql.Null[uint64]
		if err := rows.Scan(
			&file.ResultID, &logicalPath, &binaryName, &digest, &size,
		); err != nil {
			rows.Close()
			return ProjectSnapshot{}, 0, 0, fmt.Errorf("scan Java source file: %w", err)
		}
		file.LogicalPath = logicalPath.String
		file.BinaryName = binaryName.String
		file.SHA256 = digest.String
		file.SizeBytes = size.V
		if !logicalPath.Valid || !binaryName.Valid || !digest.Valid || !size.Valid ||
			!validSourceFile(file) || file.LogicalPath == previousPath {
			rows.Close()
			return ProjectSnapshot{}, 0, 0, ErrSourceUnavailable
		}
		if project.SourceSizeBytes > ^uint64(0)-file.SizeBytes {
			rows.Close()
			return ProjectSnapshot{}, 0, 0, ErrSourceUnavailable
		}
		project.Files = append(project.Files, file)
		project.SourceSizeBytes += file.SizeBytes
		previousPath = file.LogicalPath
	}
	if err := rows.Err(); err != nil {
		rows.Close()
		return ProjectSnapshot{}, 0, 0, fmt.Errorf("iterate Java source files: %w", err)
	}
	if err := rows.Close(); err != nil {
		return ProjectSnapshot{}, 0, 0, fmt.Errorf("close Java source files: %w", err)
	}
	if len(project.Files) == 0 || len(project.Files) > MaxFiles ||
		project.SourceSizeBytes > uint64(MaxSourceBytes) {
		return ProjectSnapshot{}, 0, 0, ErrSourceUnavailable
	}
	project.AnalysisProjectStatus = checkerProjectStatus(
		project.Status, project.Language,
	)
...
SELECT 1 FROM tasks WHERE id = ? AND deleted_at IS NULL`, taskID).Scan(&marker)
	...
	if err != nil {
		return fmt.Errorf("find Java analysis task: %w", err)
	}
	return nil
}
...
func validCreateRecord(record CreateRecord) bool {
	return uuidPattern.MatchString(record.RunID) &&
		...
		strings.HasPrefix(record.RequestKey, "java_analysis:") &&
		len(record.RequestKey) == len("java_analysis:")+64 &&
		sha256Pattern.MatchString(strings.TrimPrefix(record.RequestKey, "java_analysis:"))
}
...
func requireOne(result sql.Result, zero error) error {
	...
	if affected != 1 {
		return errors.New("Java analysis database mutation affected multiple rows")
	}
	return nil
}
