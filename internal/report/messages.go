...
// Analyzer display translations. Rule and diagnostic messages are produced by
// the offline C/Java checkers in English; the stored records keep the
// original message while presentation layers (HTML report, Word report, and
// the web UI) translate the stable rule/diagnostic codes to Chinese. Codes
// outside these tables fall back to the original message.

var cFindingTitles = map[string]string{
...
var javaFindingTitles = map[string]string{
...
var pythonFindingTitles = map[string]string{
...
func translateTitle(titles map[string]string, code string) (string, bool) {
	if code == "" {
		return "", false
	}
...
// displayMessage renders a translated title together with the original
// checker message when the code is known; unknown codes keep the message as-is.
func displayMessage(titles map[string]string, code string, message string) string {
	title, ok := translateTitle(titles, code)
	if !ok {
		return message
	}
...
// cFindingMessage translates a C checker finding by rule ID.
func cFindingMessage(ruleID string, message string) string {
	return displayMessage(cFindingTitles, ruleID, message)
}

// javaFindingMessage translates a Java checker finding by rule ID.
func javaFindingMessage(ruleID string, message string) string {
	return displayMessage(javaFindingTitles, ruleID, message)
}

// pythonFindingMessage translates a Python checker finding by rule ID.
func pythonFindingMessage(ruleID string, message string) string {
	return displayMessage(pythonFindingTitles, ruleID, message)
}

// diagnosticMessage translates an analyzer diagnostic by its code.
func diagnosticMessage(code string, message string) string {
