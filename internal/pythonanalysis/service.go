...
func NewService(repository Repository, config Config) (*Service, error) {
	if repository == nil || config.RunDeleter == nil {
		return nil, errors.New("Python analysis repository and run deleter are required")
	}
	if config.NewID == nil {
...
func (s *Service) Create(
	...
	runID, err := s.newID()
	if err != nil {
		return Run{}, false, fmt.Errorf("generate Python analysis run ID: %w", err)
	}
	jobID, err := s.newID()
	if err != nil {
		return Run{}, false, fmt.Errorf("generate Python analysis job ID: %w", err)
	}
	if !uuidPattern.MatchString(runID) || !uuidPattern.MatchString(jobID) ||
		runID == jobID {
		return Run{}, false, errors.New(
			"Python analysis ID generator returned invalid or duplicate UUIDs",
		)
	}
	...
	return s.repository.Create(ctx, CreateRecord{
		...
		RequestKey: "python_analysis:" + hex.EncodeToString(digest[:]),
	})
}
	...
	if page.HasMore {
		if len(page.Items) == 0 {
			return RunPage{}, errors.New(
				"Python analysis repository returned an empty page with more results",
			)
		}
		...
		if err != nil {
			return RunPage{}, fmt.Errorf("encode Python analysis cursor: %w", err)
		}
	}
	return page, nil
}
