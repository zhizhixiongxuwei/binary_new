...
func TestServiceCreateHashesIdempotencyAndAllocatesImmutableRun(t *testing.T) {
	...
	if repository.created.RunID != testRunID ||
		...
		!strings.HasPrefix(repository.created.RequestKey, "c_analysis:") ||
		len(repository.created.RequestKey) != len("c_analysis:")+64 {
...
func TestServiceDeleteDelegatesCascadeAndMapsDomainErrors(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "success"},
		{
			name: "missing", err: report.ErrCAnalysisRunNotFound,
			want: ErrRunNotFound,
		},
		{
			name: "active", err: report.ErrCAnalysisRunNotTerminal,
