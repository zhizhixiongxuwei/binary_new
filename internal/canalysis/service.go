...
func NewService(repository Repository, config Config) (*Service, error) {
	if repository == nil || config.RunDeleter == nil {
		return nil, errors.New("C analysis repository and run deleter are required")
	}
	if config.NewID == nil {
...
func (s *Service) Create(
	...
	runID, err := s.newID()
	if err != nil {
		return Run{}, false, fmt.Errorf("generate C analysis run ID: %w", err)
	}
	jobID, err := s.newID()
	if err != nil {
		return Run{}, false, fmt.Errorf("generate C analysis job ID: %w", err)
	}
	if !uuidPattern.MatchString(runID) || !uuidPattern.MatchString(jobID) ||
		runID == jobID {
		return Run{}, false, errors.New(
			"C analysis ID generator returned invalid or duplicate UUIDs",
		)
	}
	...
	return s.repository.Create(ctx, CreateRecord{
		...
		RequestKey: "c_analysis:" + hex.EncodeToString(digest[:]),
	})
}
	...
	if page.HasMore {
		if len(page.Items) == 0 {
			return RunPage{}, errors.New(
				"C analysis repository returned an empty page with more results",
			)
		}
		...
		if err != nil {
			return RunPage{}, fmt.Errorf("encode C analysis cursor: %w", err)
		}
	}
	return page, nil
}
...
func (s *Service) ListFindings(
	...
	if !validRunQuery(RunQuery{TaskID: query.TaskID, RunID: query.RunID}) ||
		...
		!validFunctionFilter(query.Function) {
...
func (s *Service) Delete(ctx context.Context, input ActionInput) error {
	...
	switch {
	case errors.Is(err, report.ErrCAnalysisRunNotFound):
		return ErrRunNotFound
	case errors.Is(err, report.ErrCAnalysisRunNotTerminal):
...
func validFunctionFilter(value string) bool {
